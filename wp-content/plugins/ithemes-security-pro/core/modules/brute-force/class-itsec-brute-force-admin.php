...
class ITSEC_Brute_Force_Admin {
	...
	public function itsec_add_dashboard_status( $statuses ) {
		...
		if ( isset( $this->settings['enabled'] ) && $this->settings['enabled'] === true && $api_ban === true ) {
			...
			$status       = array( 'text' => __( 'Your login area is protected from brute force attacks.', 'it-l10n-ithemes-security-pro' ), 'link' => '#itsec_brute_force_settings', );
			...
			$status_array = 'medium';
			$status       = array( 'text' => __( 'Your login area is partially protected from brute force attacks. We recommend you use both network and local blocking for full security.', 'it-l10n-ithemes-security-pro' ), 'link' => '#itsec_brute_force_settings', );

		} else {

			$status_array = 'high';
			$status       = array( 'text' => __( 'Your login area is not protected from brute force attacks.', 'it-l10n-ithemes-security-pro' ), 'link' => '#itsec_brute_force_settings', );
